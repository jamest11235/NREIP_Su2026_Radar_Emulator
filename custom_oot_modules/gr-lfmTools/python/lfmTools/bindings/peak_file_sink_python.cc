/*
 * Copyright 2026 Free Software Foundation, Inc.
 *
 * This file is part of GNU Radio
 *
 * SPDX-License-Identifier: GPL-3.0-or-later
 *
 */

/***********************************************************************************/
/* This file is automatically generated using bindtool and can be manually edited  */
/* The following lines can be configured to regenerate this file during cmake      */
/* If manual edits are made, the following tags should be modified accordingly.    */
/* BINDTOOL_GEN_AUTOMATIC(0)                                                       */
/* BINDTOOL_USE_PYGCCXML(0)                                                        */
/* BINDTOOL_HEADER_FILE(peak_file_sink.h)                                        */
/* BINDTOOL_HEADER_FILE_HASH(e75f8a3f733d4ddc7fc6cf9369bd9e4d)                     */
/***********************************************************************************/

#include <pybind11/complex.h>
#include <pybind11/pybind11.h>
#include <pybind11/stl.h>

namespace py = pybind11;

#include <gnuradio/lfmTools/peak_file_sink.h>
// pydoc.h is automatically generated in the build directory
#include <peak_file_sink_pydoc.h>

void bind_peak_file_sink(py::module& m)
{

    using peak_file_sink = ::gr::lfmTools::peak_file_sink;


    py::class_<peak_file_sink,
               gr::sync_block,
               gr::block,
               gr::basic_block,
               std::shared_ptr<peak_file_sink>>(m, "peak_file_sink", D(peak_file_sink))

        .def(py::init(&peak_file_sink::make),
             py::arg("file") = "delay.txt",
             D(peak_file_sink, make))


        ;
}
